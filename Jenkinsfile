    ...
    stages{
            ...
            steps{
                script{
                    withCredentials([string(credentialsId: 'docker-pass', variable: 'docker_password')]) {
                             sh '''
                                docker build -t 54.80.90.77:8083/test-app:${VERSION} .
                                docker login -u admin -p $docker_password 54.80.90.77:8083 
                                docker push  54.80.90.77:8083/test-app:${VERSION}
                                docker rmi 54.80.90.77:8083/test-app:${VERSION}
                            '''
                    }
                }
            }
                ...
                script{
                    withCredentials([usernamePassword(credentialsId: 'nexus', passwordVariable: 'nexus_password', usernameVariable: 'nexus_username')]) {
                             ...
                             sh '''
                                 ...
                                 curl -u $nexus_username:$nexus_password http://54.80.90.77:8081/repository/helm-hosted/ --upload-file myapp-${helmversion}.tgz -v
                            '''
                          }
                    }
                }
        ...
        stage('Deploying application on k8s cluster') {
            steps {
               script{
                        dir('kubernetes/') {
                          sh 'helm upgrade --install --set image.repository="54.80.90.77:8083/test-app" --set image.tag="${VERSION}" myjavaapp myapp/ ' 
                        }
